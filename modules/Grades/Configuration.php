...
if($_REQUEST['values'])
{
	DBQuery("DELETE FROM PROGRAM_USER_CONFIG WHERE USER_ID='".User('STAFF_ID')."' AND PROGRAM='Gradebook' AND SCHOOL_ID ='".$_REQUEST['sqlsyear']."'");
	foreach($_REQUEST['values'] as $title=>$value)
		DBQuery("INSERT INTO PROGRAM_USER_CONFIG (USER_ID,PROGRAM,TITLE,VALUE,SCHOOL_ID) values('".User('STAFF_ID')."','Gradebook','$title','".str_replace("\'","''",str_replace('%','',$value))."','".$_REQUEST['sqlsyear']."')");
}
