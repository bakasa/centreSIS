...
//		echo "<CENTER><h4>Are You Sure You Want to $action that $title?</h4><FORM action=$PHP_tmp_SELF&delete_ok=1 METHOD=POST><INPUT type=submit value=OK></FORM><FORM action=$PHP_tmp_SELF&delete_cancel=1 METHOD=POST><INPUT type=submit value=Cancel></FORM></CENTER>";
		echo "<CENTER><h4>"._('Are You Sure You Want to')." $action "._('that').ucwords($title)."?</h4><FORM action=$PHP_tmp_SELF METHOD=POST><INPUT type=submit name=delete_ok value="._('OK')."><INPUT type=submit name=delete_cancel value="._('Cancel')."></FORM></CENTER>";
		PopTable('footer');
		return false;
	}
