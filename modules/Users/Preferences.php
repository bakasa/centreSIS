...
if(!$_REQUEST['modfunc'])
{
	...
	if($_REQUEST['tab']=='password')
	{
		...
		echo '<TABLE><TR><TD align=right><font color=gray>'._('Current Password').'</font></TD><TD><INPUT type=password name=values[current]></TD></TR>
						<TR><TD align=right><font color=gray>'._('New Password').'</font></TD><TD><INPUT type=password name=values[verify]></TD></TR>
						<TR><TD align=right><font color=gray>'._('Verify New Password').'</font></TD><TD><INPUT type=password name=values[new]></TD></TR></TABLE>';
	}
	...
	if($_REQUEST['tab']=='staff_fields' && User('PROFILE')=='admin')
	{
		...
		ListOutput($custom_fields_RET,$columns,_('User Field'),_('User Fields'),array(),array(array('CATEGORY')));
	}
